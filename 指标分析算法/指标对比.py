...
df = merged_df.loc[merged_df["NAME_1"] == "D4"].drop(columns=["NAME_1", "name"])
df = df.fillna(-1)
df.to_csv('../指标总结/D4Test.csv', index=False, encoding='utf-8')
...
n_estimators = 91,  # 例如设置为 100 棵树
learning_rate = 0.16,  # 学习率设置为 0.1
max_depth = 7,  # 每棵树的最大深度为 3
